						...
						<scroll-view class="failureRecord-height" :scroll-y="true" :show-scrollbar="false" @scrolltolower="scrolltolower">
							<view class="failureRecord-item" v-for="(item,index) in recordList" :key="index">
								<view class="failureRecord-item__box">
									...
									<view class="failureRecord-item__content">{{ statusList[item.status] }}</text></view>
...
export default {
	...
	computed: {
		statusList() {
			return [this.i18n.status.Extraction_pending, this.i18n.status.success, this.i18n.status.fail];
		}
	},
	mounted() {
		this.init();
