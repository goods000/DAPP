					...
					<view class="popupBox-contentbg">
						...
						<scroll-view class="failureRecord-height" :scroll-y="true" @scrolltolower="scrolltolower">
							...
							<view class="failureRecord-item" v-for="(item, index) in recordList" :key="index">
								...
								<view class="failureRecord-item__content">{{ i18n.global.time }}：{{ item.createTime | formatTime('MDHM') }}
									<text class="failureRecord-item__content-record" @click="openPopup('openDetail', item.id)">{{ i18n.composite.openResult.details }}</text>
								</view>
							</view>
						</scroll-view>
					</view>
