		...
		<view class="popupBox">
			<view class="popupBox-wrapper">
				...
				<view class="popupBox-container">
					...
					<view class="popupBox-data" v-if="tabsActive == 1">
						<view class="popupBox-data__num">{{ i18n.index.pay }}：{{ boxDetails.dlbPrice | number(2) }}HBIC</view>
						<view class="popupBox-data__num">{{ i18n.index.balance }}：{{ total | number(2) }}HBIC</view>
					</view>
					<view class="popupBox-data" v-else>
...
export default {
	...
	watch:{
		quantity(newValue, oldValue) {
			console.log('newValue：',newValue,this.tabsActive);
			if(newValue == ''){
				this.total = this.quantity * ( this.tabsActive == 1 ? this.boxDetails.dlbPrice : this.boxDetails.usdtPrice);
				console.log(1,this.total);
				return;
			}else{
				this.total = newValue * ( this.tabsActive == 1 ? this.boxDetails.dlbPrice : this.boxDetails.usdtPrice);
	...
	methods: {
		...
		getBuy(){
			...
			if(this.tabsActive == 1){
				var contractAddress = this.$store.state.platmentContractAddress;
				var contractDecimal = this.$store.state.platmentContractDecimal;
			}else{
				var contractAddress = this.$store.state.usdtContractAddress;
				var contractDecimal = this.$store.state.usdtContractDecimal;
			}
			...
			let parameter = [
				...
				{
					type: 'uint256',
					// value: this.total * Math.pow(10, contractDecimal)
					value: this.$BigNumber(this.total).times(Math.pow(10, contractDecimal)).toFixed()
				}
			];
			// console.log(this.tabsActive,"contractAddress：",contractAddress,"contractDecimal：",contractDecimal,);
			this.$tronWeb.triggerSmartContract(this.$store.state.userWalletAddress, contractAddress, 'transfer(address,uint256)', parameter).then(result => {
