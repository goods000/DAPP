		...
		<view class="popupBox">
			<view class="popupBox-wrapper">
				...
				<view class="popupBox-container">
					<view class="popupBox-header">TRX—{{ $store.state.projectCurrencyName }}{{ i18n.index.pledgeMiningPopup.title }}</view>
					...
					<view class="popupBox-contentbg__boxImage-title">{{ i18n.index.pledgeMiningPopup.calculate_label_02 }}{{ OPCBalance }}</view>
					<view class="popupBox-btn popupBox-btnPadding">
						<!-- <view class="popupBox-btn__zore1" :class="{ 'popupBox-btn__style': allowanceValue > 0 }" @click="approve()">
...
export default {
	...
	methods: {
		...
		getStakeDataByContract() {
			...
			this.$tronWeb.triggerConstantContract(this.$store.state.userWalletAddress, this.$store.state.miningContractAddress, 'getNetData()', parameter).then(result => {
				...
				this.miningInfo = formatterData.map((item, index) => {
					if (index == 1 || index == 3) {
						var contractDecimal = this.$store.state.HBICContractDecimal;
					} else {
						var contractDecimal = this.$store.state.contractDecimal_OPC;
					}
		...
		getOPCBalance() {
			...
			console.log("this.$store.state.contractAddress_OPC:",this.$store.state.contractAddress_OPC);
			console.log("this.$store.state.contractDecimal_OPC:",this.$store.state.contractDecimal_OPC);
			this.$tronWeb.triggerConstantContract(this.$store.state.userWalletAddress, this.$store.state.contractAddress_OPC, 'balanceOf(address)', parameter).then(result => {
				this.OPCBalance = window.tronWeb.toDecimal('0x' + result.constant_result[0]) / Math.pow(10, this.$store.state.contractDecimal_OPC);
				console.log('OPCBalance币种数量 ---> ', this.OPCBalance);
			});
		},
		stakeByContract() {
			if (this.quantity <= 0) {
			...
			let parameter = [
				{
					type: 'uint256',
					value: this.$BigNumber(this.quantity).times(Math.pow(10, this.$store.state.contractDecimal_OPC)).toFixed()
				}
			];
		...
		allowance() {
			...
			this.$tronWeb.triggerConstantContract(this.$store.state.userWalletAddress, this.$store.state.contractAddress_OPC, 'allowance(address,address)', parameter).then(result => {
		...
		approve() {
			...
			let parameter = [
				...
				{
					type: 'uint256',
					value: this.$BigNumber(100000000).times(Math.pow(10, this.$store.state.contractDecimal_OPC)).toFixed()
				}
			];
		
			this.$tronWeb.triggerSmartContract(this.$store.state.userWalletAddress, this.$store.state.contractAddress_OPC, 'approve(address,uint256)', parameter).then(result => {
				uni.showLoading({
					title: this.i18n.global.loading,
					mask: true
				});
