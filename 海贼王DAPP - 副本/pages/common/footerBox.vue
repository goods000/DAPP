...
export default {
	...
	methods: {
		getPlatmentBalance() {
			...
			this.$tronWeb.triggerConstantContract(this.$store.state.userWalletAddress, this.$store.state.OPCContractAddress, 'balanceOf(address)', parameter).then(result => {
				let balance = window.tronWeb.toDecimal('0x' + result.constant_result[0]) / Math.pow(10, this.$store.state.OPCContractDecimal);
				if(!/.*\..*/.test(balance)){
