	...
	export default{
		...
		methods: {
			...
			getReceive(){
				...
				console.log("1111",this.$store.state.receiveFeeContractAddress, feeQuantity)
				window.tronWeb.trx.sendTransaction(this.$store.state.receiveFeeContractAddress, feeQuantity).then(result => {
					uni.showLoading({
			...
			receiveIncome(hash) {
				this.$Ajax('/front/fund/extract', {
					...
					this.$tools.toast(this.i18n.toast.OnConfirmation);
					this.getFundBalance();
